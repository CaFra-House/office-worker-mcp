...
| Executive report (premium design) | Invoice (Word → PDF pipeline) | Multi-chapter book (auto TOC) | Excel with pivot table |
|:---:|:---:|:---:|:---:|
| ![](docs/demo-informe.png) | ![](docs/demo-factura.png) | ![](docs/demo-manual.png) | ![](docs/demo-excel.png) |

- **Informe** — `render_document` with `design_mode="premium"` + corporate theme
- **Factura** — `create_word` (itemized table) → `convert_to_pdf`, full Word→PDF flow
- **Manual** — `create_book`: cover + auto-numbered TOC + 4 chapters in one call
- **Excel** — `edit_excel add_pivot`: real dynamic pivot sheet aggregated from raw data
...
## How we compare (competitive landscape, verified via GitHub API — Sep 2026)

We are a **new project**, so we don't compete on star count — we compete on the gap nobody fills: a **single, local-first, portable, enterprise-grade document MCP** that covers the full lifecycle. Here's the field, with real numbers pulled from the GitHub API:
...
| [OfficeMCP](https://github.com/OfficeMCP/OfficeMCP) | 116 | Python/.NET COM | **Windows only** | 2025-05-28 ⚠️ stale ~1y | Drives real MS Office via COM + `RunPython` (arbitrary code execution). Doesn't run on Linux/Mac; not maintained. |
...
> Honest positioning: we win on the *gap*, not on existing adoption. Distribution (Phase B: MCP directories, Show HN / Reddit, README demos) is what converts that gap into installs — not more features.
